...
export default function getNews(req, res) {
  ...
  async function sendNews(agent) {
    ...
    agent.add(
      `<p style="margin:0; font-size:1.2rem; font-weight:bolder; background-color:#e3e3e3; padding:10px; margin-buttom:10px; border-radius:8px;">${randomNews.headline}</p> <p style="margin:0;"> ${randomNews.summary}</p>`
    );
  }
